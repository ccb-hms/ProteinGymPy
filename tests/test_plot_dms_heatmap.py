...
class TestPlotDMSHeatmap(unittest.TestCase):
    """Test suite for plot_dms_heatmap module."""
    ...
    def setUp(self):
        ...
        self.sample_dms_data = {'TEST_ASSAY': df}
    ...
    def test_filter_by_pos(self):
        ...
        self.assertEqual(filtered['pos'].min(), 3)
        self.assertEqual(filtered['pos'].max(), 10)
        
        # Test with only start
        filtered = _filter_by_pos(df, start_pos=5)
        self.assertEqual(filtered['pos'].min(), 5)
        self.assertEqual(filtered['pos'].max(), 12)
        
        # Test with only end
        filtered = _filter_by_pos(df, end_pos=5)
        self.assertEqual(filtered['pos'].min(), 1)
        self.assertEqual(filtered['pos'].max(), 5)
    
    def test_filter_by_pos_errors(self):
        ...
        with self.assertRaisesRegex(ValueError, "start_pos.*outside the assay range"):
            _filter_by_pos(df, start_pos=10)
        
        # Test with end_pos outside range
        with self.assertRaisesRegex(ValueError, "end_pos.*outside the assay range"):
            _filter_by_pos(df, end_pos=0)
        
        # Test missing pos column
        df_no_pos = pd.DataFrame({'value': range(5)})
        with self.assertRaisesRegex(ValueError, "must contain a 'pos' column"):
            _filter_by_pos(df_no_pos)
    
    def test_filter_exact_coord(self):
        ...
        self.assertEqual(list(result['pos']), [1, 2, 3, 4, 5])
        self.assertTrue(pd.isna(result[result['pos'] == 2]['value'].iloc[0]))
        self.assertTrue(pd.isna(result[result['pos'] == 4]['value'].iloc[0]))
        
        # Test with exact_coord=False
        result = _filter_exact_coord(df, exact_coord=False)
        self.assertEqual(len(result), 3)
        self.assertEqual(list(result['pos']), [1, 3, 5])
    
    def test_make_colormap_dms(self):
        ...
        self.assertEqual(vmin, -2)
        self.assertEqual(vmax, 2)
    ...
    def test_plot_dms_heatmap_eve_colors(self):
    ...
    def test_plot_dms_heatmap_exact_coord(self):
    ...
    def test_plot_dms_heatmap_invalid_assay(self):
        """Test error handling for invalid assay name."""
        with self.assertRaisesRegex(ValueError, "not found in dms_data"):
            plot_dms_heatmap(
                assay_name='NONEXISTENT_ASSAY',
                dms_data=self.sample_dms_data
            )
    
    def test_plot_dms_heatmap_only_multiple_sites(self):
        ...
        with self.assertRaisesRegex(ValueError, "contains only multiple amino acid sites"):
            plot_dms_heatmap(
                assay_name='MULTI_SITE_ASSAY',
                dms_data=data
            )
    
    def test_plot_dms_heatmap_clustering(self):
    ...
    def test_plot_dms_heatmap_clustering_with_na(self):
        ...
        try:
            ...
            with self.assertRaisesRegex(ValueError, "missing values, preventing clustering"):
                plot_dms_heatmap(
                    assay_name='TEST',
                    dms_data=data,
                    start_pos=1,
