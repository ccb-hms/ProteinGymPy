...
def _filter_alphamissense_table(
    ...
    """
    Filter AlphaMissense table for a specific DMS assay.
    ...
    Returns:
        Filtered DataFrame for the specified DMS assay
    """
    if am_table is None:
    ...
    # Filter for dms_id
    filtered_table = am_table[am_table['DMS_id'] == dms_id].copy()

    # Check if table is empty after filtering
    if len(filtered_table) == 0:
        raise ValueError(
            f"No AlphaMissense information found for DMS assay '{dms_id}'. "
            f"Check that the DMS_id is correct."
        )
...
def _filter_model_table(
    ...
    """
    Filter model table (zero-shot or supervised) for a specific DMS assay.
    ...
    Returns:
        DataFrame for the specified DMS assay
    """
    # Check if dms_id exists in model_table
    if dms_id not in model_table:
        raise ValueError(
            f"No model predictions found for DMS assay '{dms_id}'. "
            f"Check that the DMS_id is correct."
        )

    return model_table[dms_id].copy()


def _filter_dms_table(
    dms_table: Dict[str, pd.DataFrame],
    dms_id: str
) -> pd.DataFrame:
    """
    Filter DMS table for a specific assay.

    Args:
        dms_table: Dictionary of DMS DataFrames (one per assay)
        dms_id: DMS assay identifier

    Returns:
        DataFrame for the specified DMS assay
    """
    # Check if dms_id exists in dms_table
    if dms_id not in dms_table:
        raise ValueError(
            f"No DMS data found for assay '{dms_id}'. "
            f"Check that the DMS_id is correct."
        )

    return dms_table[dms_id].copy()
...
def _merge_dms_model_tables(
    dms_df: pd.DataFrame,
    model_df: pd.DataFrame,
    model: str
) -> pd.DataFrame:
    """
    Merge DMS and model dataframes by mutant.
    ...
    Returns:
        Merged DataFrame with DMS_score and model score columns
    """
    # Check that DMS_id is the same
    dms_id_dms = dms_df['DMS_id'].unique()
    dms_id_model = model_df['DMS_id'].unique()

    if len(dms_id_dms) != 1 or len(dms_id_model) != 1 or dms_id_dms[0] != dms_id_model[0]:
        raise ValueError("DMS_id must be the same across both tables")

    # Merge tables on mutant
    merged_table = pd.merge(
        dms_df,
        model_df,
        on='mutant',
        how='inner',
        suffixes=('_dms', '_model')
    )

    # Select relevant columns
    cols = ['DMS_id_dms', 'UniProt_id_dms', 'mutant', 'DMS_score', model]
    ...
    merged_table = merged_table.rename(columns={
        'DMS_id_dms': 'DMS_id',
        'UniProt_id_dms': 'UniProt_id'
    })

    # Remove NaN values
    merged_table = merged_table.dropna(subset=['DMS_score', model])
...
def _calculate_spearman_correlation(
    ...
    """
    Calculate Spearman correlation between DMS scores and model predictions.

    Args:
        merged_table: Merged DataFrame with DMS_score and model score columns
        model: Model name
...
def dms_corr_plot(
    ...
    relationship between experimental DMS scores and model predictions.

    Args:
        dms_id: Valid DMS assay identifier (e.g., "BRCA1_HUMAN_Findlay_2018")
        model: Model to compare (default: "AlphaMissense")
    ...
    Examples:
        >>> # Use default (AlphaMissense)
        >>> fig = dms_corr_plot(dms_id="BRCA1_HUMAN_Findlay_2018")
        >>> plt.show()

        >>> # Compare with specific model
        >>> fig = dms_corr_plot(
        ...     dms_id="TP53_HUMAN_Giacomelli_2018",
        ...     model="GEMME"
        ... )
        >>> plt.show()
    ...
    # Validate dms_id
    if not isinstance(dms_id, str):
        raise ValueError("dms_id must be a string")
    ...
    # Load DMS data
    print(f"Loading DMS data for {dms_id}...")
    dms_data = get_dms_substitution_data(cache_dir=cache_dir)
    dms_df = _filter_dms_table(dms_data, dms_id)

    # Load model data
    print(f"Loading {model} predictions...")
    model_df = _get_model_dataframe(model, dms_id, cache_dir)

    # Merge tables
    print("Merging DMS scores with model predictions...")
    merged_table = _merge_dms_model_tables(
        dms_df=dms_df,
        model_df=model_df,
        model=model
    )
    ...
    if len(merged_table) == 0:
        raise ValueError(
            f"No common mutants between DMS data and {model} predictions for '{dms_id}'"
        )

    print(f"Found {len(merged_table)} common mutants")

    # Calculate correlation
    correlation, pvalue = _calculate_spearman_correlation(merged_table, model)
    ...
    # Create hexbin plot
    x = merged_table['DMS_score'].values
    y = merged_table[model].values
    ...
    # Style main plot
    ax_main.set_xlabel('DMS score', fontsize=14)
    ax_main.set_ylabel(f'{model} score', fontsize=14)
...
def dms_corr_plot(
    ...
    dms_df = _filter_dms_table(dms_table, uniprot_id, cache_dir)
