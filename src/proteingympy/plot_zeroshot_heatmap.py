"""
plot_zeroshot_heatmap.py - Python equivalent of plot_zeroshot_heatmap.R

Visualize zero-shot model scores along a protein as a heatmap.
"""
...
def _make_colormap_model(mat: np.ndarray, color_scheme: str = "default") -> Tuple[Any, float, float, float]:
    """
    Create a colormap for the heatmap.
    
    Args:
        mat: Matrix of model scores
        color_scheme: Either "default" (parula-like) or "EVE" (popEVE portal colors)
        
    Returns:
        Tuple of (colormap, vmin, vmid, vmax)
    """
    ...
    if color_scheme == "EVE":
        ...
        positions = [0, (halfpt - min_val) / (max_val - min_val), 
                    (mid_val - min_val) / (max_val - min_val), 1]
        cmap = LinearSegmentedColormap.from_list(
            "eve", list(zip(positions, colors))
        )
    else:
        # Default: Parula-like colormap (MATLAB's parula: blue -> cyan -> yellow)
        # Approximation of parula colors
        colors = ["#352A87", "#0363E1", "#1485D4", "#06A7C6", "#38B99E", 
                  "#92BF73", "#D9BA56", "#FCCE2E", "#F9FB0E"]
        cmap = LinearSegmentedColormap.from_list("parula", colors)
    
    return cmap, min_val, mid_val, max_val
...
def plot_zeroshot_heatmap(
    ...
    **kwargs
    ...
    """
    Visualize zero-shot model scores along a protein.
    
    Plots predicted model scores under the zero-shot model for amino acid 
    substitutions along a protein in a defined DMS assay. The x-axis shows 
    amino acid positions, and the y-axis represents possible amino acid residues, 
    ordered by default based on physiochemical groupings.
    
    Args:
        assay_name: Valid assay name (key in model_data dictionary). For the full 
            list of available assays, check the keys of the loaded data.
        model: Name of the zero-shot model to plot. Must be a column in the 
            assay dataframe. To view the list of models, check available columns 
            in the data (typically 79 zero-shot models).
        model_data: Dictionary of zero-shot assays with DataFrames. If None, 
            attempts to load from proteingympy data loading functions.
        ...
        exact_coord: If True, plot precise start_pos and end_pos coordinates, 
            filling missing positions with NaN. If False, plot only positions 
            with available data.
        ...
        color_scheme: "default" for parula-like colors or "EVE" for popEVE portal colors
    ...
    Raises:
        ValueError: If assay contains only multiple amino acid sites, model not found, 
            or invalid parameters
        ImportError: If required packages are missing
        
    Examples:
        >>> from proteingympy.make_zero_shot_substitutions import get_zero_shot_substitution_data
        ...
        >>> # With EVE color scheme and clustering
        >>> fig, ax = plot_zeroshot_heatmap(
        ...     assay_name="ACE2_HUMAN_Chan_2020",
        ...     model="EVE",
        ...     model_data=model_data,
    ...
    # Load model_data if not provided
    if model_data is None:
        print(
            "'model_data' not provided, "
            "attempting to load zero-shot model data..."
        )
        try:
            from .make_zero_shot_substitutions import get_zero_shot_substitution_data
            model_data = get_zero_shot_substitution_data()
        except ImportError:
            raise ImportError(
                "Could not import get_zero_shot_substitution_data. "
                "Please provide model_data parameter."
            )
    
    # Extract the specified assay
    if assay_name not in model_data:
        raise ValueError(
            f"Assay '{assay_name}' not found in model_data. "
            f"Available assays: {list(model_data.keys())[:5]}..."
        )
    
    assay_df = model_data[assay_name].copy()
    ...
    if len(assay_df) == 0:
        raise ValueError(
            f"Unable to plot zero shot substitution heatmap; "
            f"assay '{assay_name}' contains only multiple amino acid sites."
        )
    
    # Check if model exists in the dataframe
    if model not in assay_df.columns:
        raise ValueError(
            f"Model '{model}' not found in assay '{assay_name}'. "
            f"Available models: {[col for col in assay_df.columns if col not in ['mutant', 'mutated_sequence', 'UniProt_id', 'DMS_id', 'DMS_score', 'DMS_score_bin']][:5]}..."
        )
    
    # Select chosen model
    assay_df = assay_df[['mutant', model]]
    ...
    # Create colormap
    cmap, vmin, vmid, vmax = _make_colormap_model(reordered_matrix, color_scheme)
